...
class MiniDipole(ThreeDScene):
    def construct(self):
        ...
        mag_field = MagneticField(
            wire1, x_range=[0, 0], y_range=[-7, 7], z_range=[-4, 4]
        )
...
class MovingChargeMagneticField(ThreeDScene):
    def construct(self):
        ...
        mag_field = MagneticField(w1, x_range=[0, 0], y_range=[-4, 4], z_range=[-4, 4])
...
class BiotSavartLaw(Scene):
    def construct(self):
        ...
        cross_prod_magnitude = MathTex(
            r"|d\vec{l}", r"\times", r"\vec{r}|", "=", "dl", "r", r"sin(\theta)"
        ).to_edge(UR)
        vector_a = (
            Vector(2 * UP + LEFT)
            .next_to(cross_prod_magnitude, DOWN)
            .shift(0.5 * DOWN + LEFT)
        )
        vector_a.set_color(RED)
        vector_b = Vector(2 * UP + 2 * RIGHT).put_start_and_end_on(
            start=vector_a.get_start(), end=vector_a.get_start() + 2 * UP + 2 * RIGHT
        )
        vector_b.set_color(RED)

        theta_arc = Arc(
            radius=1, start_angle=PI / 4, angle=np.arctan2(2, -1) - PI / 4
        ).move_arc_center_to(vector_a.get_start())
...
class RightHandRule(Scene):
    def construct(self):
        ...
        cross_eqn = MathTex(r"\vec{A}", r"\times", r"\vec{B}", "=", r"\vec{C}").to_edge(
            UR
        )
        sin_eqn = MathTex(
            "|",
...
class RightHandRuleCurrentBField(ThreeDScene):
    def construct(self):
        ...
        hand = SVGMobject("svg/right-hand-rule.svg").scale(2)
        wire1 = (
            Vector(10 * LEFT + 3 * UP).shift(5 * RIGHT + 1.5 * DOWN).set_color(YELLOW_A)
        )
        ...
        for i in range(7):
            ...
            charge_imgs.append(
                VGroup(new_charge.image, new_charge.text).move_to(wire1.get_start())
            )
        ...
        self.play(
            AnimationGroup(
                *[MoveAlongPath(c1, wire1) for c1 in charge_imgs], lag_ratio=0.1
            )
        )
        ...
        rotation_mobs = VGroup(
            b_copy, b_point, hand, r_label, r_vector, b_label, b_vector
        )
...
class StraightLineCurrent(ThreeDScene):
    def construct(self):
        ...
        mag_field = MagneticField(
            mag_wire1, x_range=[-4, 4], y_range=[0, 0], z_range=[-4, 4]
        )
...
class VectorsInOut(Scene):
    def construct(self):
        in_vector = VGroup(
            ...
            Line(start=0.25 * DL, end=0.25 * UR, color=WHITE),
            Line(start=0.25 * DR, end=0.25 * UL, color=WHITE),
        )
...
class LorenzDirectionExample(ThreeDScene):
    def construct(self):
        ...
        b_field = (
            ArrowVectorField(lambda pos: 2 * RIGHT, x_range=[-4, 4], y_range=[-2, 2])
            .set_color(BLUE_D)
            .set_opacity(0.7)
        )
...
class VCrossBPointsUp(Scene):
    def construct(self):
        ...
        label2 = MathTex("q", r"\vec{v}", r"\times", r"\vec{B}").next_to(
            down_arrow, RIGHT
        )
...
class PathTracingExample(Scene):
    def construct(self):
        ...
        b_label = (
            Tex(r"$\vec{B}$").next_to(mag_field, UR).shift(0.25 * DL).set_color(BLUE)
        )
        ...
        c1_stuff.add_updater(
            lambda mobject: mobject.move_to(
                2.5 * np.cos(theta.get_value()) * RIGHT
                + 2.5 * np.sin(theta.get_value()) * UP
            )
        )
        velocity1.add_updater(
            lambda mobject: mobject.put_start_and_end_on(
                2.5 * np.cos(theta.get_value()) * RIGHT
                + 2.5 * np.sin(theta.get_value()) * UP
                + -0.7 * np.sin(theta.get_value()) * RIGHT
                + 0.7 * np.cos(theta.get_value()) * UP,
                2.5 * np.cos(theta.get_value()) * RIGHT
                + 2.5 * np.sin(theta.get_value()) * UP
                + -1.7 * np.sin(theta.get_value()) * RIGHT
                + 1.7 * np.cos(theta.get_value()) * UP,
            )
        )
        v1_label.add_updater(
            lambda mobject: mobject.move_to(velocity1.get_end()).shift(
                0.2 * np.cos(theta.get_value()) * RIGHT
                + 0.2 * np.sin(theta.get_value()) * UP
                - 0.2 * np.sin(theta.get_value()) * RIGHT
                + 0.2 * np.cos(theta.get_value()) * UP
            )
        )
        force1.add_updater(
            lambda mobject: mobject.put_start_and_end_on(
                1.8 * np.cos(theta.get_value()) * RIGHT
                + 1.8 * np.sin(theta.get_value()) * UP,
                0.8 * np.cos(theta.get_value()) * RIGHT
                + 0.8 * np.sin(theta.get_value()) * UP,
            )
        )
        f1_label.add_updater(
            lambda mobject: mobject.move_to(force1.get_end()).shift(
                0.2 * np.cos(theta.get_value()) * RIGHT
                + 0.2 * np.sin(theta.get_value()) * UP
                - 0.5 * np.sin(theta.get_value()) * RIGHT
                + 0.5 * np.cos(theta.get_value()) * UP
            )
        )
...
class NorthernLights(ThreeDScene):
    def construct(self):
        ...
        v1 = (
            Vector(0.33 * (4 * UP + RIGHT), color=RED)
            .next_to(c1.image, UR)
            .shift(0.2 * DL)
        )
        v_para = Vector().put_start_and_end_on(
            start=v1.get_start(), end=v1.get_start() + 0.33 * RIGHT
        )
        v_perp = Vector().put_start_and_end_on(
            start=v_para.get_end(), end=v_para.get_end() + 0.33 * 4 * UP
        )
        ...
        corkscrew = ParametricFunction(
            lambda t: cork_speed * t * RIGHT
            + radius * np.sin(t) * UP
            + radius * np.cos(t) * OUT,
            t_range=[-25, 25],
        )
        ...
        self.play(
            Write(corkscrew), FadeOut(c1.elements, v1, v_para, v_perp), run_time=4
        )
        self.wait()
...
class wire1OfChargeLorenzDefinition(Scene):
    def construct(self):
        ...
        lorenz_law = MathTex(
            r"\vec{F}", "=", r"Q_{\text{total}}", r"\vec{v}", r"\times", r"\vec{B}"
        )
        lorenz_law.scale(1.25).next_to(w_and_label, DOWN)

        eqn2 = MathTex(
            r"\vec{F}", "=", "{n}", "q", "A", "l", r"\vec{v}", r"\times", r"\vec{B}"
        )
        eqn2.scale(1.25).next_to(lorenz_law, DOWN)

        eqn3 = MathTex(
            r"\vec{F}", "=", "{n}", "q", "A", "{v}", r"\vec{l}", r"\times", r"\vec{B}"
        )
        ...
        eqn5 = MathTex(
            r"\vec{F}", "=", r"\int", "I", r"d\vec{l}", r"\times", r"\vec{B}"
        )
...
class HallVoltage(Scene):
    def construct(self):
        ...
        b_field.shift(2 * UP)
        b_label = (
            Tex(r"$\vec{B}$").next_to(b_field, UR).shift(0.25 * DL).set_color(BLUE)
        )
...
class Parallelwire1sExample(ThreeDScene):
    def construct(self):
        ...
        top_mag_field = MagneticField(
            Wire(top_wire1), x_range=[0, 0], y_range=[-3, 3], z_range=[-3, 3]
        )

        bottom_mag_field = MagneticField(
            Wire(bottom_wire1), x_range=[0, 0], y_range=[-3, 3], z_range=[-3, 3]
        )

        mag_field = top_mag_field + bottom_mag_field

        top_b_field = VGroup(Circle(radius=0.5, color=BLUE), Dot(color=BLUE))
...
class RailgunExample(Scene):
    def construct(self):
        ...
        b_label = (
            Tex(r"$\vec{B}$").next_to(b_arrow, UR).shift(0.25 * DL).set_color(BLUE)
        )
        ...
        self.wait()
        self.play(
            moving_stuff.animate.shift(25 * LEFT),
...
class Motor3DExample(ThreeDScene):
    def construct(self):
        loop = (
            VGroup(
                Line(start=DOWN, end=DOWN + 2 * OUT),
        ...
        i_label = (
            Tex(r"$I$", color=YELLOW_E)
            .next_to(loop[-1].get_end(), UP + IN)
            .rotate(PI / 2, RIGHT)
            .rotate(PI / 2, OUT)
        )
...
class Motor2DExample(Scene):
    def construct(self):
        ...
        eqn7 = MathTex(
            r"\vec{\tau_{\text{loop}}}", "=", "w", "I", "{l}", "{B}", r"sin(\theta)"
        )
        ...
        eqn9 = MathTex(
            r"\vec{\tau_{\text{loop}}}", "=", r"\vec{\mu}", r"\times", r"\vec{B}"
        )
        ...
        current_in = VGroup(
            Circle(radius=0.5).set_color(YELLOW_C), Dot().set_color(YELLOW_C)
        ).move_to(top_wire1.get_start())

        current_out = VGroup(
            Circle(radius=0.5).set_color(YELLOW_C),
            Line(start=0.25 * DL, end=0.25 * UR, color=YELLOW_C),
            Line(start=0.25 * UL, end=0.25 * DR, color=YELLOW_C),
        ).move_to(top_wire1.get_end())

        f1 = Vector(2.5 * UP, color=RED).put_start_and_end_on(
            current_in.get_center(), current_in.get_center() + 2.5 * UP
        )
        f2 = Vector(2.5 * DOWN, color=RED).put_start_and_end_on(
            current_out.get_center(), current_out.get_center() + 2.5 * DOWN
        )

        f1_label = Tex(r"$\vec{F}$", color=RED).next_to(f1.get_end(), UP)
        f2_label = Tex(r"$\vec{F}$", color=RED).next_to(f2.get_end(), DOWN)

        t1_arc = Arc(radius=1, start_angle=PI / 4, angle=PI / 4).move_arc_center_to(
            f1.get_start()
        )
        t2_arc = Arc(
            radius=1, start_angle=3 * PI / 2 - PI / 4, angle=PI / 4
        ).move_arc_center_to(f2.get_start())
...
class GaussLawMagnetic(Scene):
    def construct(self):
        ...
        law_eqn = MathTex(r"\oint", r"\vec{B}", r"\cdot", r"d\vec{A}", "=", "0").scale(
            1.5
        )
...
class PathIntegral(Scene):
    def construct(self):
        ...
        wiggly_line = ParametricFunction(
            wiggle_func, t_range=np.array([-PI, PI]), color=BLUE
        )

        stretched_points = np.array(
            [stretch_func(point) for point in wiggly_line.points]
        )
        ...
        that_one_field_vector = (
            Vector(RIGHT).move_to(dl_vector.get_start()).shift(0.5 * RIGHT)
        )
        ...
        vec2_angle = 0
        vec1 = Vector(RIGHT, color=BLUE).put_start_and_end_on(
            ORIGIN,
            ORIGIN
            + radius * np.cos(vec1_angle) * RIGHT
            + radius * np.sin(vec1_angle) * UP,
        )
        vec1.move_to(UR)
        vec2 = Vector(RIGHT, color=BLUE).put_start_and_end_on(
            ORIGIN,
            ORIGIN
            + radius * np.cos(vec2_angle) * RIGHT
            + radius * np.sin(vec2_angle) * UP,
        )
...
class AmpereExampleEqns(Scene):
    def construct(self):
        ...
        eqn5 = MathTex(
            "B", "=", r"{\mu_0", r"I_{\text{thru}}", r"\over", "2", r"\pi", "{r}}"
        )
        eqn5.next_to(eqn4, DOWN)
...
class MagneticFieldStrengthGraph(Scene):
    def construct(self):
        ...
        labels = ax.get_axis_labels(
            y_label=Tex("$B$").set_color(BLUE), x_label=Tex("$r$").set_color(RED)
        )
...
class FaradaysLaw(Scene):
    def construct(self):
        ...
        self.wait()
        self.play(
            FadeOut(raw_flux, raw_flux2, raw_flux3), the_law.animate.move_to(ORIGIN)
        )
        self.play(the_law.animate.scale(1.5))
        self.wait()
...
class EddyCurrents(Scene):
    def construct(self):
        ...
        b_arrow = VGroup(
            Circle(radius=0.5),
            Line(start=0.25 * DL, end=0.25 * UR),
            Line(start=0.25 * UL, end=0.25 * DR),
        )
        b_arrow.set_color(BLUE)

        metal = (
            Rectangle(color=GRAY, height=2, width=3)
            .set_fill(color=color_gradient([BLACK, GRAY], 2))
            .set_opacity(0.7)
        )
        metal.shift(4 * LEFT)
        ...
        circ_arrow = (
            SVGMobject("svg/arrow-circle.svg")
            .set_color(YELLOW)
            .scale(0.5)
            .move_to(metal)
        )
        move_up = (
            Vector(0.75 * UP).shift(1.25 * LEFT + (0.75 / 2) * DOWN).set_color(WHITE)
        )
        f_left = Vector(color=RED).put_start_and_end_on(
            metal.get_left() + 0.2 * LEFT, metal.get_left() + 1.2 * LEFT
        )
        ...
        move_up2 = (
            Vector(0.75 * UP).shift(1.25 * RIGHT + (0.75 / 2) * DOWN).set_color(WHITE)
        )
        f_left2 = Vector(color=RED).put_start_and_end_on(
            metal.get_right() + 1.2 * RIGHT, metal.get_right() + 0.2 * RIGHT
        )
...
class Transformer(Scene):
    def construct(self):
        ...
        iron = (
            Rectangle(color=GRAY, height=4, width=1)
            .set_fill(color=color_gradient([BLACK, GRAY], 2))
            .set_opacity(0.7)
        )
        coil1 = VGroup(
            Line(start=1.5 * UP, end=1.5 * UP + 3 * LEFT),
        ...
        self.wait()
        self.play(
            iron.animate.shift(3 * LEFT),
            coil1.animate.shift(3 * LEFT),
            coil2.animate.shift(3 * LEFT),
        )
        self.wait()

        b_arrow = Vector(color=BLUE).put_start_and_end_on(
            start=coil1[0].get_start() + UP, end=coil2[0].get_start() + DOWN
        )
        ...
        eqn3.next_to(eqn2, DOWN)
        eqn4 = MathTex(
            r"|V_1|", "=", "N_1", r"{d", r"\phi_{1,\text{loop}}", r"\over", "d", "t}"
        )
        eqn4.next_to(eqn3, DOWN)
        eqn5 = MathTex(
            r"{|V_1|",
        ...
        eqn5.move_to(eqn4)
        eqn6 = MathTex(
            r"|V_2|", "=", "N_2", r"{d", r"\phi_{2,\text{loop}}", r"\over", "d", "t}"
        )
        eqn6.next_to(eqn5, DOWN)
        eqn7 = MathTex(
            r"|V_2|", "=", "N_2", r"{d", r"\phi_{1,\text{loop}}", r"\over", "d", "t}"
        )
        eqn7.move_to(eqn6)
...
class MagnetismTypes(Scene):
    def construct(self):
        ...
        def get_random_vec():
            return Vector(
                (random.random() - 0.5) * RIGHT + (random.random() - 0.5) * UP
            )
        ...
        mag1 = Vector(color=BLUE).put_start_and_end_on(
            loop1.get_center(), loop1.get_center() + LEFT
        )
        mag2 = Vector(color=BLUE).put_start_and_end_on(
            loop2.get_center(), loop2.get_center() + RIGHT
        )

        self.play(Write(loop1), Write(loop2), GrowArrow(mag1), GrowArrow(mag2))
        self.wait()
        self.play(
            GrowArrow(b_field.copy().shift(UP)),
            GrowArrow(b_field.shift(DOWN)),
            FadeIn(b_label.shift(DOWN)),
        )
        self.wait()
        self.play(
            VGroup(loop1, mag1).animate.scale(1.5),
...
class Inductor(Scene):
    def construct(self):
        ...
        eqn2 = MathTex(r"V", "=", "-", "L", "{d", "I", r"\over", "d", "t}").next_to(
            eqn1, DOWN
        )
...
class InductorWaves(Scene):
    def construct(self):
        ...
        v0_ticks = VGroup(
            Line(start=3.8 * LEFT + UP, end=3.4 * LEFT + UP),
            Line(start=3.8 * LEFT + DOWN, end=3.4 * LEFT + DOWN),
        )
        v0_labels = VGroup(
            Tex("$V_0$").next_to(v0_ticks[0], LEFT),
            Tex("$-V_0$").next_to(v0_ticks[1], RIGHT),
        )

        i0_ticks = VGroup(
            Line(start=3.8 * LEFT + 2 * UP, end=3.4 * LEFT + 2 * UP),
            Line(start=3.8 * LEFT + 2 * DOWN, end=3.4 * LEFT + 2 * DOWN),
        )
        i0_labels = VGroup(
            Tex("$I_0$").next_to(i0_ticks[0], LEFT),
        ...
        self.wait()
        self.play(
            Transform(amps_graph, amps_graph2),
            i0_labels[1].animate.shift(1.5 * LEFT),
...
class InductorPhasor(Scene):
    def construct(self):
        ...
        x_c_phasor = Vector(
            -phasor_radii * np.sin(time.get_value()) * RIGHT
            + phasor_radii * np.cos(time.get_value()) * UP
        ).set_color(ORANGE)
        x_c_phasor.add_updater(
            lambda mobject: mobject.set_angle(time.get_value() + PI / 2)
        )
        ...
        x_l_phasor = Vector(
            0.5 * phasor_radii * np.sin(time.get_value()) * RIGHT
            - 0.5 * phasor_radii * np.cos(time.get_value()) * UP
        ).set_color(PURPLE)
        x_l_phasor.add_updater(
            lambda mobject: mobject.set_angle(time.get_value() - PI / 2)
        )
        ...
        r_phasor = Vector(
            (phasor_radii / 1.5) * np.cos(time.get_value()) * RIGHT
            + (phasor_radii / 1.5) * np.sin(time.get_value()) * UP
        ).set_color(RED)
        r_phasor.add_updater(lambda mobject: mobject.set_angle(time.get_value()))

        r_height_line = Line().set_color(ORANGE)
        r_height_line.add_updater(
            lambda mobject: mobject.put_start_and_end_on(
                r_phasor.get_end(), r_phasor.get_end()[1] * UP
            )
        )
        ...
        z_phasor = Vector((phasor_radii / 1.5) * RIGHT + phasor_radii * UP).set_color(
            YELLOW_A
        )
        z_phasor.add_updater(
            lambda mobject: mobject.set_angle(
                np.arctan2(
                    x_c_phasor.get_y() + x_l_phasor.get_y() + r_phasor.get_y(),
        ...
        z_label.add_updater(
            lambda mobject: mobject.next_to(z_phasor.get_end(), UR).shift(0.25 * DL)
        )

        impedance_eqn = (
            MathTex("Z", "=", r"\sqrt{", "X^2", "+", "R^2", "}").to_edge(UR).shift(DOWN)
        )
        impedance_eqn.set_color_by_tex("Z", YELLOW_A)
        impedance_eqn.set_color_by_tex("X", ORANGE)
        impedance_eqn.set_color_by_tex("R", RED)

        impedance_eqn2 = MathTex(
            "Z", "=", r"\sqrt{", "(", "X_C", "-", "X_L", ")^2", "+", "R^2", "}"
        ).move_to(impedance_eqn)
        ...
        ohm_law_ac = MathTex(r"V_{\text{rms}}", "=", r"I_{\text{rms}}", "Z").next_to(
            impedance_eqn2, DOWN
        )
        ...
        self.play(
            ...
            Write(x_c_phasor),
            Write(x_l_phasor),
            FadeIn(r_label, x_c_label, x_l_label),
        )
        self.wait()
...
class InductorEnergy(Scene):
    def construct(self):
        ...
        eqn2 = MathTex(
            "I", "V", "-", "I^2", "R", "-", "L", "I", "{dI", "\\over", "dt}", "=", "0"
        )
        eqn2.next_to(eqn1, DOWN)

        eqn3 = MathTex(
            "P", "=", "{dU", "\over", "dt}", "=", "L", "I", "{dI", "\over", "dt}"
        )
        eqn3.next_to(eqn2, DOWN)
        ...
        graph = (
            FunctionGraph(lambda t: (1 - np.e ** (-t)), x_range=[-1, 4, 1], color=GREEN)
            .next_to(circuit, DOWN)
            .shift(DOWN + 2 * RIGHT)
        )

        y_axis = Line(
            start=graph.get_start() + 0.2 * DOWN, end=graph.get_start() + 3 * UP
        )
        x_axis = Line(
            start=graph.get_start() + 0.2 * LEFT, end=graph.get_start() + 5 * RIGHT
        )
        ...
        q_tick = Line(
            start=graph.get_start() + 2.75 * UP + 0.25 * LEFT,
