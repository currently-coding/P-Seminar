...
class MaxwellsEquations(Scene):
    def construct(self):
        ...
        maxwell_eqns1 = VGroup(*[header for header in headers], *[eqn for eqn in eqns])
...
class DisplacementCurrent(Scene):
    def construct(self):
        ampere_eqn1 = MathTex(
            ...
            "I_d",
            ")",
        )  # ,"+",r"\mu_0",r"\epsilon_0","{d",r"\phi_E",r"\over","d","{t}}")
        ...
        d_current = MathTex(
            "I_d", "=", r"\epsilon_0", "{d", r"\phi_E", r"\over", "d", "{t}}"
        )
...
class ACLightWave(Scene):
    def construct(self):
        ...
        c1.elements.add_updater(
            lambda mobject: mobject.move_to(
                pole.get_start() + UP * charge_height.get_value()
            )
        )
        ...
        self.wait()
        self.play(
            charge_height.animate.set_value(6),
        ...
        self.play(
            AnimationGroup(
                *[Write(circs[i + 3]) for i in range(len(circs) - 3)], lag_ratio=0.2
            )
        )
        self.wait()
...
class ThreeFieldsAndWaves(Scene):
    def construct(self):
        ...
        for i in range(round(2 * PI / d_theta)):
            ...
            new_arrow.shift(
                1.2 * np.cos(i * d_theta) * RIGHT + 1.2 * np.sin(i * d_theta) * UP
            )
        ...
        wave = ParametricFunction(lambda t: (t, np.sin(t), 0), t_range=(0, 4 * PI))
...
class SpeedOfLight(MovingCameraScene):
    def construct(self):
        ...
        text_mobjects = (
            VGroup(*text_array1, *text_array2)
            .scale(0.75)
        ...
        self.play(
            AnimationGroup(
                FadeIn(text_mobjects),
                self.camera.frame.animate.move_to(text_mobjects.get_bottom()),
                lag_ratio=0.2,
                run_time=5,
            ),
        ...
        self.wait(0.5)
        self.play(
            FadeOut(*text_array1, *text_array2[0:-1]), text_array2[-1].animate.scale(2)
        )
        self.wait()
    ...
    def construct(self):
        ...
        wave_graph = always_redraw(
            lambda: FunctionGraph(
                lambda x: wave_func(x, time.get_value()), x_range=[-5, 5]
            )
        )

        start_line = Line(start=5 * LEFT, end=5 * RIGHT, color=YELLOW)
        endpoints = VGroup(
            Dot(color=YELLOW).move_to(start_line.get_start()),
        ...
        follow_line_start = wave_start_dot.add_updater(
            lambda mobject: mobject.move_to(line_copy.get_start())
        )

        wave_end_dot = Dot(color=YELLOW).move_to(line_copy.get_end())
        follow_line_end = wave_end_dot.add_updater(
            lambda mobject: mobject.move_to(line_copy.get_end())
        )

        wave_start_line = Line(
            start=start_line.get_start(), end=wave_start_dot.get_center() + 0.01 * RIGHT
        )
        wave_start_line.add_updater(
            lambda mobject: mobject.put_start_and_end_on(
                start_line.get_start(), wave_start_dot.get_center() + 0.01 * RIGHT
            )
        )

        wave_end_line = Line(
            start=start_line.get_end(), end=wave_end_dot.get_center() + 0.01 * RIGHT
        )
        wave_end_line.add_updater(
            lambda mobject: mobject.put_start_and_end_on(
                start_line.get_end(), wave_end_dot.get_center() + 0.01 * RIGHT
            )
        )
        ...
        wave_start_dot.add_updater(
            lambda mobject: mobject.move_to(wave_graph.get_start())
        )
...
class WaveEquation(Scene):
    def construct(self):
        equation = MathTex(
            r"\frac{\partial^2 u}{dt^2} = c^2 \frac{\partial^2 u}{\partial x^2}"
        ).shift(UP)
        equation2 = MathTex(
            "c",
            "=",
...
class WaveEquationSolution(Scene):
    def construct(self):
        ...
        wave_number = MathTex("k", "=", "{2", r"\pi", r"\over", r"\lambda}").next_to(
            wave_solution, DOWN
        )
        frequency = (
            MathTex("f", "=", r"\frac{\text{cycles}}{\text{second}}")
            .to_edge(UP)
            .shift(2.5 * DOWN)
        )
        angular_frequency = (
            MathTex(r"\omega", "=", "2", r"\pi", "f").to_edge(UP).shift(4 * DOWN)
        )
        ...
        self.wait()
        self.play(
            wave_solution.animate.to_edge(UP),
        ...
        self.wait()
        self.play(
            FadeOut(wave_solution, wave_number, frequency, angular_frequency, speed)
        )
...
class Intensity(Scene):
    def construct(self):
        ...
        equation2 = MathTex("I", "=", "{U", r"\over", "t", "A}").next_to(
            equation1, DOWN
        )
...
class AntennaTransmitter(Scene):
    def construct(self):
        ...
        intensity_2 = MathTex(
            "I", "=", "{P", r"\over", "A}", "=", "{P", r"\over", "4", r"\pi", "d^2}"
        ).next_to(cell_tower, RIGHT)
        intensity_3 = MathTex(
            "I",
            "=",
            "{P",
        ...
        voltage_def = MathTex(
            "|V|", "=", r"\int", r"\vec{E}", r"\cdot", r"d\vec{l}"
        ).next_to(e_0_solved, DOWN)
        v_max = MathTex(r"|V_{\text{max}}|", "=", "E_0", "{l}").next_to(
            voltage_def, DOWN
        )
        v_max2 = MathTex(
            r"|V|_{\text{max}}",
        ...
        e_b_planewave = (
            MathTex("{E_0", r"\over", "B_0}", "=", "{c}").scale(0.75).to_edge(UP)
        )
        b_wave_eqn = (
            MathTex(
                "B",
        ...
        max2 = MathTex(
            ...
            "{r}^2",
            r"_\text{max}",
        ).next_to(max_voltage, DOWN)
        max3 = MathTex(
            "=", "{E_0", r"\over", "c}", "2", r"\pi", "f", r"\pi", "{r}^2"
        ).next_to(max2, DOWN)
...
class WaveMomentum(Scene):
    def construct(self):
        momentum_equation = MathTex(
            r"{U}_{\text{per\_volume}}", "=", r"{p}_{\text{per\_volume}}", "{c}"
        )
        p_mv = MathTex("{p}", "=", "{m}", "{v}")
        n2l = MathTex("{F}", "=", "{m}", "{a}")
        n2l2 = MathTex(
            "{F}", "=", "{m}", "{a}", "=", "{d", "{p}", r"\over", "d", "{t}}"
        )
        ...
        derivation_mobjects = (
            VGroup(p_mv, n2l, n2l2, p_accumulate, u_int, u_int2, u_int3, u_int4, u_int5)
            .arrange_submobjects(DOWN)
            .to_edge(UP)
        )

        momentum_eqn2 = MathTex(
            r"\epsilon_0", "{E}_0^2", "=", r"{p}_{\text{per\_volume}}", "{c}"
        )
        momentum_eqn3 = MathTex(
            r"{\epsilon_0",
            "{E}_0^2",
            r"\over",
            "{c}}",
            "=",
            r"{p}_{\text{per\_volume}}",
        )
        momentum_eqn3_v2 = MathTex(
            r"{\epsilon_0",
            "{E}_0^2",
            r"\over",
            "{c}}",
        ...
        volume2 = MathTex("{d{V}", r"\over", "dt}", "=", "{A}", "{c}").next_to(
            volume1, DOWN
        )

        pressure_eqns = VGroup(
            momentum_eqn2,
            momentum_eqn3,
            momentum_eqn3_v2,
            momentum_eqn4,
        ...
        pressure_eqns.scale(0.75).arrange(DOWN).next_to(
            momentum_equation.copy().to_corner(UL).shift(RIGHT), DOWN
        )

        cube_polygons = [
            # top face
            [[0, 0.5, 0], [-1, 1, 0], [0, 1.5, 0], [1, 1, 0]],
            # left face
            [[0, 0.5, 0], [0, -0.75, 0], [-1, -0.25, 0], [-1, 1, 0]],
            # right face
            [[0, 0.5, 0], [1, 1, 0], [1, -0.25, 0], [0, -0.75, 0]],
        ]
...
class Bonk(MovingCameraScene):
    def construct(self):
        ...
        for i in range(round(run_time / delta_t)):
            self.play(
                light.animate.shift(light_velocity * delta_t), run_time=delta_t / 2
            )
            self.play(
                Write(
                    Line(
                        start=light.get_center() - light_velocity * delta_t,
                        end=light.get_center(),
                        color=WHITE,
                    )
                ),
                run_time=delta_t / 2,
...
class Reflect(Scene):
    def construct(self):
        interface_material = (
            ...
            .shift(4 * UP)
            .set_fill(color=color_gradient([BLUE, BLACK], 2), opacity=0.4)
        )
        ...
        theta_i_arc = Arc(
            radius=1, start_angle=5 * PI / 4, angle=PI / 4 + 0.2
        ).move_arc_center_to(incident_vector.get_end())
        theta_o_arc = Arc(
            radius=1.5, start_angle=3 * PI / 2 - 0.1, angle=PI / 4 + 0.1
        ).move_arc_center_to(outgoing_vector.get_start())
        ...
        self.play(
            AnimationGroup(
                Write(interface_material), Write(perp_line), lag_ratio=0.2, run_time=1
            )
        )
        self.play(
            AnimationGroup(
                Write(incident_vector), Write(outgoing_vector), lag_ratio=0.2
            ),
            run_time=1,
        )
        self.wait()
...
class Scatter(Scene):
    def construct(self):
        ...
        incoming_vecs = VGroup(
            *[
                Vector(2 * RIGHT + 3 * UP, color=RED).shift((i + 1) * LEFT + 3 * DOWN)
                for i in range(5)
            ]
        )
        outgoing_vecs = VGroup(
            *[
                Vector(3 * RIGHT, color=BLUE)
                .set_angle(PI + random.random() * PI)
                .shift((i - 1) * LEFT + UP)
                for i in range(5)
            ]
        )
        ...
        self.play(
            AnimationGroup(Write(incoming_vecs), Write(outgoing_vecs), lag_ratio=0.2)
        )
        self.wait()
    ...
    def construct(self):
        corner = (
            VGroup(
                ...
                DashedLine(start=LEFT + 0.75 * DOWN, end=LEFT + 0.75 * UP),
            )
            .rotate(PI / 4)
            .scale(2)
            .shift(3 * RIGHT + DOWN)
        ...
        incoming_vec = Vector().put_start_and_end_on(
            4 * LEFT + 2 * UP, corner[1].get_center()
        )
        middle_vec = Vector().put_start_and_end_on(
            incoming_vec.get_end(), corner[0].get_center()
        )
        outgoing_vec = Vector().put_start_and_end_on(
            corner[0].get_center(), incoming_vec.get_start() + middle_vec.height * DOWN
        )

        vecs = VGroup(incoming_vec, middle_vec, outgoing_vec).set_color_by_gradient(
            RED, BLUE
        )

        self.play(Write(corner[0:4]))
        self.play(
            AnimationGroup(Write(incoming_vec), Write(corner[4]), lag_ratio=0.4),
            run_time=1,
        )
        self.play(
            AnimationGroup(Write(middle_vec), Write(corner[5]), lag_ratio=0.4),
            run_time=1,
        )
...
class Refract(Scene):
    def construct(self):
        ...
        outgoing_vector = Vector(5 * RIGHT + 2 * UP, color=BLUE).shift(
            1.2 * UP + 0.2 * RIGHT
        )
        ...
        theta_o_arc = Arc(
            radius=1,
            start_angle=np.arctan2(2, 5),
            angle=PI / 2 - np.arctan2(2, 5) + 0.2,
        ).move_arc_center_to(outgoing_vector.get_start())
        theta_i_label = (
            Tex(r"$\theta_i$")
            .next_to(theta_i_arc, DOWN)
            .shift(0.5 * LEFT)
            .set_color(GRAY)
        )
        theta_o_label = (
            Tex(r"$\theta_o$")
            .next_to(theta_o_arc, UP)
            .shift(0.5 * RIGHT)
            .set_color(GRAY)
        )
        ...
        snells_law = MathTex(
            "n_1", "sin(", r"\theta_i", ")", "=", "n_2", "sin(", r"\theta_o", ")"
        )
        ...
        self.play(
            AnimationGroup(Write(interface_material), Write(perp_line), lag_ratio=0.2)
        )
        self.play(
            AnimationGroup(
                Write(incident_vector), Write(outgoing_vector), lag_ratio=0.5
            )
        )
...
class TotalInternalRefraction(Scene):
    def construct(self):
        ...
        self.wait()
        self.play(
            incident_vector.animate.rotate(-PI / 6 - 0.2, OUT, UP),
            outgoing_vector.animate.rotate(-PI / 6 - 0.2, OUT, UP),
            run_time=3,
        )
        self.wait()
...
class IndexOfRefraction(Scene):
    def construct(self):
        ...
        equation2 = (
            MathTex("n", "=", "{c", r"\over", "v}").scale(1.5).next_to(equation, DOWN)
        )
...
class UnderwaterExample(Scene):
    def construct(self):
        sine_wave = FunctionGraph(
            lambda x: 0.2 * np.sin(x) + 1, x_range=[-7, 7], color=BLUE
        )
        ...
        water_IOR = (
            Tex("$n=1.33$", color=BLUE_D).to_edge(LEFT, buff=0.2).shift(0.5 * UP)
        )
        ...
        theta_i_arc = Arc(
            radius=1, start_angle=PI + PI / 4, angle=PI / 4 + 0.2
        ).move_arc_center_to(incident_vector.get_end())
        theta_o_arc = Arc(
            radius=1.2, start_angle=3 * PI / 2 - 0.2, angle=PI / 4 + 0.2
        ).move_arc_center_to(outgoing_vector.get_start())
        ...
        snells_law3 = MathTex(
            r"\displaystyle", "1.33", r"\cdot", "sin(", r"\theta_i", ")", "=", "1"
        ).move_to(snells_law2)
        snells_law4 = MathTex(
            r"\displaystyle", "sin(", r"\theta_i", ")", "=", r"\frac{1}{1.33}"
        ).move_to(snells_law2)
        snells_law5 = MathTex(
            r"\displaystyle",
            r"\theta_i",
            "=",
            r"arcsin(\frac{1}{1.33})",
            r"\approx",
            r"48.6 ^{\circ}",
        ).move_to(snells_law)
        snells_law_flip = MathTex(
            r"\displaystyle", r"\theta_i", "=", r"arcsin(\frac{1.33}{1})"
        ).move_to(snells_law)
...
class ConstructiveInterference(Scene):
    def construct(self):
        wave1 = FunctionGraph(
            lambda x: 0.8 * np.sin(x), x_range=[-7, 7], color=WHITE
        ).shift(2 * UP)
        wave2 = FunctionGraph(
            lambda x: 0.8 * np.sin(x), x_range=[-7, 7], color=WHITE
        ).shift(2 * DOWN)
        wave3 = FunctionGraph(
            lambda x: 2.4 * np.sin(x), x_range=[-7, 7], color=WHITE
        )  # technical lie but easier to see
...
class DestructiveInterference(Scene):
    def construct(self):
        ...
        wave2 = FunctionGraph(
            lambda x: 0.8 * np.sin(x + PI), x_range=[-7, 7], color=WHITE
        ).shift(2 * DOWN)
        wave3 = FunctionGraph(
            lambda x: 0, x_range=[-7, 7], color=WHITE
        )  # technical lie but easier to see
...
class DoubleSlit(Scene):
    def construct(self):
        ...
        constructive = Tex(r"Constructive: $\Delta r = m \lambda$").next_to(
            delta_r, DOWN
        )
        destructive = Tex(
            r"Destructive: $\Delta r = (m+ \frac{1}{2}) \lambda$"
        ).next_to(constructive, DOWN)
        ...
        waves_inc = (
            VGroup(
                wave.copy().scale(0.5).shift(4 * LEFT),
                ...
                wave.copy().scale(2).shift(LEFT),
            )
            .scale(1.25)
            .shift(0.45 * LEFT)
        )
        waves_out1 = VGroup(
            wave.copy().scale(0.5).shift(1.5 * UP + 0.45 * RIGHT),
            wave.copy().scale(0.75).shift(1.5 * UP + 0.7 * RIGHT),
        )
        waves_out2 = VGroup(
            wave.copy().scale(0.5).shift(1.5 * DOWN + 0.45 * RIGHT),
...
class BraggDiffraction(Scene):
    def construct(self):
        dots = [
            Dot(color=WHITE).set_z_index(10),
        ...
        incoming_vecs = VGroup(
            Vector(4 * RIGHT + 2 * DOWN, color=BLUE).shift(4.1 * LEFT + 2.1 * UP),
        ...
        dotted_lines = VGroup(
            DashedLine(start=ORIGIN, end=0.75 * (LEFT + 2 * DOWN)),
        ...
        theta_arcs = VGroup(
            Arc(
                radius=0.5,
                start_angle=3 * PI / 2 - np.arctan2(1, 2),
