...
class MyScene(ThreeDScene):
    ...
    def create_chain_and_seat_template(self):
        ...
        chain_end = chain_start * (
            1
            + math.sin(self.theta.get_value())
            * self.chain_length
            / self.carousel_radius
        ) + np.array([0, 0, -math.cos(self.theta.get_value()) * self.chain_length])
        direc = chain_end - chain_start
        chain = Cylinder(
            radius=0.02, height=self.chain_length, direction=direc, color=WHITE
        ).move_to(chain_start + direc / 2)
    ...
    def create_carousel(self):
        ...
        base = Circle(radius=self.carousel_radius, color=BLUE).set_fill(
            WHITE, opacity=0.3
        )

        chains_and_seats = VGroup()
        template = self.create_chain_and_seat_template().rotate(
            self.angle, about_point=ORIGIN, axis=OUT
        )
    ...
    def construct(self):
        ...
        def update_carousel(mob, dt):
            self.seat_radius = self.carousel_radius + self.chain_length * math.sin(
                self.theta.get_value()
            )
            self.angular_speed = (
                9.81 * math.tan(self.theta.get_value()) / self.seat_radius
            ) ** 0.5
        ...
        self.play(
            Transform(
                self.info_tex,
                self.adjust_tex(MathTex(r"\tan{\theta}=\frac{m\omega^2r}{mg}")),
            )
        )
        self.wait(1)
        self.play(
            Transform(
                self.info_tex,
                self.adjust_tex(MathTex(r"\tan{\theta}=\frac{\omega^2r}{g}")),
            )
        )
        self.wait(1)
        ...
        self.play(
            Transform(
                self.info_tex, self.adjust_tex(MathTex(r"g\tan{\theta}=\omega^2r"))
            )
        )
        self.wait(1)
        self.play(
            Transform(
                self.info_tex,
                self.adjust_tex(MathTex(r"\frac{g\tan{\theta}}{r}=\omega^2")),
            )
        )
        self.wait(1)
        self.play(
            Transform(
                self.info_tex,
                self.adjust_tex(MathTex(r"\sqrt{\frac{g\tan{\theta}}{r}}=\omega")),
            )
        )
        self.wait(1)
        self.play(
            Transform(
                self.info_tex,
                self.adjust_tex(MathTex(r"\omega=\sqrt{\frac{g\tan{\theta}}{r}}")),
            )
        )
        self.wait(1)
        self.play(
            Transform(
                ...
                self.adjust_tex(
                    MathTex(
                        r"\omega=",
                        r"\sqrt{\frac{9.81\cdot\tan{"
                        + f"{self.theta.get_value():.5f}"
                        + "}}{"
                        + f"{self.seat_radius:.5f}"
                        + "}}="
                        + f"{self.angular_speed:.5f}",
                    )
                ),
            )
        )
        self.wait(1)
        self.info_tex.add_updater(
            lambda tex: tex.become(
                self.adjust_tex(
                    MathTex(
                        r"\omega=\sqrt{\frac{9.81\cdot\tan{"
                        + f"{self.theta.get_value():.3f}"
                        + "}}{"
                        + f"{self.seat_radius:.3f}"
                        + "}}="
                        + f"{self.angular_speed:.3f}"
                    )
                )
            )
        )
